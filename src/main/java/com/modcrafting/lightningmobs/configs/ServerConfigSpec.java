...
public class ServerConfigSpec {
	...
	public static final ForgeConfigSpec.DoubleValue WEATHER_CHANGE_CHANCE;
	
	public static final ConfigValue<List<? extends String>> UPGRADEABLE_MOBS;
	
	static {
		BUILDER.push("Looting");
		SHARDS_CHANCE = BUILDER.comment("Chances for shards to drop. Looting adds an accumulative 25% of this value (25%, 75%, 150%)")
				.defineInRange("shards_chance", Double.valueOf(0.02), 0, 1);
		SHARDS_CHANCE_AMOUNT = BUILDER.comment("Chances for shards when drop to give extra loot. If chances are given, 50% or less of the looting level will be dropped extra, half this value for upper 50%. Looting also multiplies the chance.")
				.defineInRange("shards_chance_amount", Double.valueOf(0.1), 0, 1);
		BUILDER.pop();
		BUILDER.push("Events");
		WEATHER_CHANGE_CHANCE = BUILDER.comment("")
				.defineInRange("weather_change_chance", Double.valueOf(0.15), 0, 1);
		BUILDER.pop();
